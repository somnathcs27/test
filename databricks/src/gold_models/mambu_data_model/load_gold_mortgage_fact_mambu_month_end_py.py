...
if datetime.now(tz).day == 1:
    try:
        fact = FactFactory(
            ...
            table_name='fact_mortgage_account',
            source_query=None,
            periodic_snapshot_table_name='fact_mortgage_account_history'
    ...
    try:
        ...
        catalog_name=catalog_name,
        table_schema='gold_con',
        table_name='fact_mortgage_part',
        source_query=None,
        periodic_snapshot_table_name='fact_mortgage_part_history'
        ).create(FactType.PERIODIC_SNAPSHOT_MONTH)
        fact.load()
    except Exception as e:
        dbutils.jobs.taskValues.set("error_detail", str(e))
