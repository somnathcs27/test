...
@dlt.materialized_view(
    name="transformed_mambu_loan_account", 
    comment="Transforms Mambu loan account data with calculated balloon dates, current rank, benchmark rates, and rank types based on product configurations."
)
@dlt.expect("Check that ENCODED_KEY is not null", "ENCODED_KEY IS NOT NULL")
def transformed_mambu_loan_account():

    df = spark.sql(
        f"""
    SELECT
        mla.ENCODED_KEY,
        cfvp.PRODUCT_ID_LA as PRODUCT_CODE,
        CASE 
